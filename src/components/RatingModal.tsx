...
export default function RatingModal({ open, onClose, dishId, dishName, initialStars, initialComment, initialPhotoUrl }: Props) {
  ...
  const submit = useMutation({
    ...
    onSuccess: () => {
      ...
      // Play success animation, then close after a beat
      setShowSuccess(true)
      setTimeout(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-ink-900/85 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-ink-800 border border-ink-700 w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-6 max-h-[90vh] overflow-y-auto relative">
        {showSuccess ? (
          <div className="flex flex-col items-center justify-center py-16">
            <div className="w-24 h-24 rounded-full bg-lime-500 text-ink-900 flex items-center justify-center animate-pop shadow-glow-lime">
              <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                <polyline points="20 6 9 17 4 12" />
              </svg>
            </div>
            <p className="display-xl text-3xl text-ink-100 mt-6 animate-fade-up">הדירוג נשמר!</p>
            <p className="text-sm text-ink-400 mt-2 animate-fade-up">
              <span className="text-lime-500 font-black tabular">{stars}</span>/5 כוכבים ל-{dishName}
            </p>
          </div>
        ) : (
          <>
            ...
            <div className="bg-ink-900 rounded-2xl border border-ink-700 p-5 flex flex-col items-center my-4">
              ...
              {stars > 0 && (
                <p className="display-xl text-4xl text-lime-500 tabular mt-3">
                  {stars}<span className="text-ink-500 text-xl">/5</span>
                </p>
              )}
            </div>
